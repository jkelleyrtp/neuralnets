...
pub fn load_trial_data(input: &[u8]) -> Result<ExpData> {
    // Load the mat file
    let mat_file = MatFile::parse(input.as_ref())?;
    ...
    for trial in data_buffer.iter_mut() {
        for x in 0..NUM_SAMPLES {
            trial[0][x] = (*data.next().unwrap() / max);
            trial[1][x] = (*data.next().unwrap() / max);
            trial[2][x] = (*data.next().unwrap() / max);
        }
    }
...
#[test]
fn plot_single() -> Result<()> {
    use RpsLabel::*;

    Ok(())
    // let exp = RawExperiment {
    //     data: load_trial_data(include_bytes!("../data/rps/EMGdata-RPS-04231538.mat"))?,
    // g.by(10)
}
